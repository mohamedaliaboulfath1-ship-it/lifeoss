# LifeOS Pro — Backup & Recovery Guide
...
- Migrations **005** and **006** must be applied (relational tables exist).
...
Archives freeze a year's metrics and payload snapshot for historical comparison.
...
- `yearly_snapshots` — frozen payload
- `yearly_summaries` — computed metrics

List archives: `GET /api/archive`

Preview year metrics: `GET /api/archive?preview=2024`
...
- **Pro tier:** Point-in-Time Recovery (PITR) — restore to any second within retention window.
...
Supabase Storage objects (avatars, book covers, progress photos, book media) are **not** included in SQL dumps. Use:

- Supabase Storage API bulk download, or
- Replicate bucket to S3 (custom script / future automation)
...
2. Identify affected tables (goals, tasks, etc.).
3. Delete corrupted rows via Supabase Table Editor (user-scoped) or SQL.
...
2. No database action required if schema unchanged.

### Scenario C — Database failure / accidental DROP

1. Supabase → Database → Backups → Restore (or PITR).
2. Verify migrations 001–009 applied post-restore.
3. Redeploy Vercel app; verify `/api/data` and auth.

### Scenario D — Full platform migration

1. `pg_dump` source database.
2. Create new Supabase project.
3. Restore dump; update Vercel env vars to new project URL/keys.
...
| Export round-trip | Export JSON → verify file opens and contains goals/habits |
...
| Data | Suggestion |
|------|------------|
| User JSON exports | Monthly, keep 3 copies (local + cloud) |
| Platform SQL dumps | Weekly, 30-day retention minimum |
| Year archives | Permanent (user-controlled) |
| Activity logs | 90 days operational review; archive older to cold storage |
...
- `life_years.payload` is empty by design — do not rely on it for recovery.
- Career/learning POST-only APIs may require re-seeding after restore (`ensureCareerSeed`).
