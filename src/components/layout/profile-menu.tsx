...
export function ProfileMenu() {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        <span className="w-8 h-8 rounded-full bg-gradient-to-br from-gold to-purple flex items-center justify-center text-sm font-bold text-white shrink-0">
          {name.charAt(0)}
        </span>
      ...
      {open && (
        <div className="absolute left-0 top-full mt-2 w-56 bg-surface border border-border2 rounded-[10px] shadow-xl z-[100] py-2 animate-fade-up">
          <div className="px-4 py-2 border-b border-border mb-1">
            ...
            <div className="text-[10px] text-text3 font-mono">مركز الحساب</div>
          </div>
          {MENU_ITEMS.map((item) => (
            <Link
              key={item.href}
