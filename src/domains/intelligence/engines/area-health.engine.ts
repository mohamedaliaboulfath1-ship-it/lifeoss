import type { AreaScoreInput } from "@/domains/intelligence/types";
import { Score } from "@/domains/shared/value-objects/score.vo";

/** Single source of truth for area health scoring */
export class AreaHealthEngine {
  calculate(input: AreaScoreInput): { score: Score; reasons: string[] } {
    ...
    return { score: new Score(score), reasons: reasons.slice(0, 4) };
  }
}

export const areaHealthEngine = new AreaHealthEngine();

/** Backward-compatible function export */
export function calcAreaHealthScore(input: AreaScoreInput): { score: number; reasons: string[] } {
  const result = areaHealthEngine.calculate(input);
  return { score: result.score.value, reasons: result.reasons };
}

export function scoreLabel(score: number): string {
  return new Score(score).label();
}
